...
export default function CreateInterruptionCard() {
  ...
  const handleDelete = async (id: string, description: string) => {
    if (!confirm(`Are you sure you want to delete this interruption?`)) return;
              ...
              >
                <div className="flex-1 space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground space-y-1">
                    ...
                    <div>
                      {interruption.interruptedFeeders.length} feeders affected
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-1">
                    {interruption.interruptedFeeders.slice(0, 3).map((item) => (
                      <Badge
                        key={item.feeder.id}
                        variant="secondary"
                        className="text-xs"
